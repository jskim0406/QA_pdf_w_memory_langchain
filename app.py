...
def main():
    ...
    api_key = st.text_input("Enter Open AI Key.", placeholder="sk-...", type="password")
    ...
    with st.sidebar:
        ...
        chat_model = st.selectbox(label="Chat Model", options=["gpt-3.5-turbo"])
        ...
        st.markdown(
            """
            **Code:**\n
            [*Github Repo*](https://github.com/jskim0406/QA_pdf_w_memory_langchain)

            **Credit:**\n
            ["How to Build a Personalized PDF Chat Bot with Conversational Memory"](https://medium.com/@avra42/how-to-build-a-personalized-pdf-chat-bot-with-conversational-memory-965280c160f8)(Avra)
            """
        )
    ...
    if loaded_file:
        ...
        llm_chain = LLMChain(
            llm=ChatOpenAI(
                temperature=temperature, openai_api_key=api_key, model_name=chat_model
            ),
            prompt=prompt,
        )
        ...
        agent = ZeroShotAgent(
            llm_chain=llm_chain, tools=tools, verbose=True  # Ret QA chain,
        )
